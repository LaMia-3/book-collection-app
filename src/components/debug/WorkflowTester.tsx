...
const getRandomCompletionDate = (): string => {
  ...
  const dateString = new Date(randomTimestamp).toISOString();
  console.log('Generated completion date:', dateString);
  return dateString;
};

// Helper to ensure we use the same date for both completedDate and dateCompleted fields
const getCompletionDateForStatus = (status: string): string | undefined => {
  if (status === 'completed') {
    return getRandomCompletionDate();
  }
  return undefined;
};

// Generate a random rating between 1-5 stars
// For completed books, always generate a rating
// For reading books, 50% chance of having a rating
// For want-to-read books, no rating
const getRandomRatingForStatus = (status: string): number | undefined => {
  if (status === 'completed') {
    // Completed books always get a rating between 1-5
    return Math.floor(Math.random() * 5) + 1;
  } else if (status === 'reading') {
    // Reading books have 50% chance of having a rating
    if (Math.random() > 0.5) {
      return Math.floor(Math.random() * 5) + 1;
    }
  }
  // Want-to-read books don't get ratings
  return undefined;
};
...
const SERIES_DEFINITIONS = [
  {
    ...
    genre: ['fantasy'],
    status: 'completed',
    searchTerms: ['Fellowship of the Ring', 'Two Towers', 'Return of the King']
  },
  {
    name: 'Mistborn',
  ...
  },
  {
    name: 'Foundation',
    author: 'Isaac Asimov',
    description: 'Science fiction series about the decline and fall of a galactic empire',
  ...
  },
  {
    ...
    description: 'Epic fantasy series set in a world where magic exists but can only be used by women',
    genre: ['fantasy'],
    status: 'completed',
    searchTerms: ['Eye of the World', 'Great Hunt', 'Dragon Reborn']
  },
  {
    ...
    genre: ['fantasy'],
    status: 'completed',
    searchTerms: ['Lion, the Witch', 'Prince Caspian', 'Dawn Treader']
  },
  {
...
export function WorkflowTester() {
  ...
  const runWorkflowTest = async () => {
    ...
    try {
      addResult("Test Started", true, "Starting workflow test");
      
      // Step 1: Add books from Google Books API
      addResult("Step 1", true, "Adding books from Google Books API");
      const googleBooks = await addBooksFromGoogleAPI();
      
      // Step 2: Add books from Open Library API
      addResult("Step 2", true, "Adding books from Open Library API");
      const openLibraryBooks = await addBooksFromOpenLibrary();
      
      // Step 3: Create series and assign books
      addResult("Step 3", true, "Creating series and assigning books");
      await createAndAssignSeries();
      
      // Step 4: Verify data consistency
      addResult("Step 4", true, "Verifying data consistency");
      await verifyData();
      
      // Test completed
      addResult("Test Completed", true, `Successfully added ${googleBooks.length + openLibraryBooks.length} books and ${Object.keys(seriesMap).length} series`);
      
    } catch (error) {
      console.error("Test failed:", error);
      addResult("Test Failed", false, `${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setRunning(false);
      setProgress(100);
    }
    ...
    for (let i = 0; i < GOOGLE_BOOK_SEARCHES.length; i++) {
      try {
        ...
        // Books 5-14 as "completed"
          ...
          status = 'completed';
          console.log(`Google book ${i+1} (${query}) will be marked as completed`);
        }
        ...
        const bookToSave = {
          ...
          // Add completion date for completed books
          completedDate: getCompletionDateForStatus(status),
          // Also set dateCompleted for the adapter layer
          dateCompleted: getCompletionDateForStatus(status),
          // Add rating based on reading status
          rating: getRandomRatingForStatus(status)
        };
    ...
    for (let i = 0; i < OPEN_LIBRARY_BOOK_SEARCHES.length; i++) {
      try {
        ...
        // Books 5-14 as "completed"
          ...
          status = 'completed';
          console.log(`Open Library book ${i+1} (${query}) will be marked as completed`);
        }
  ...
  const createAndAssignSeries = async () => {
    addResult("Series Creation", true, "Creating series from books...");
    const newSeriesMap: Record<string, string[]> = {};
    
    // Get all books
    const allStoredBooks = await enhancedStorageService.getBooks();
    
    // Create each series
    for (const seriesDef of SERIES_DEFINITIONS) {
      try {
        // Find books that match this series
        const seriesBooks = allStoredBooks.filter(book => 
        ...
        if (seriesBooks.length === 0) {
          addResult(`Series: ${seriesDef.name}`, false, "No books found for this series");
          continue;
        }
  ...
  const verifyData = async () => {
    try {
      ...
      addResult("Data Verification", true, `Found ${books.length} books and ${series.length} series in the database`);
    } catch (error) {
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      ...
                      {!book.completedDate && book.status === 'completed' && (
